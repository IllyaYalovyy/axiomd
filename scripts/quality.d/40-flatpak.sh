...
# 4. The probes. `--test-threads=1`: each one starts a compositor and a sandboxed
#    application, and two of those competing for the machine is a slow test pretending
#    to be a flaky one. `--nocapture` so what the packaged application said reaches
#    whoever ran the gate.
output=$(mktemp)
trap 'rm -f "${output}"' EXIT

cargo test -p axiomd-app --test packaging -- \
    --ignored --nocapture --test-threads=1 2>&1 | tee "${output}" || true

summary=$(grep -E '^test result:' "${output}" || true)
[[ -n "${summary}" ]] ||
    fail "the packaging probes did not finish. Their output is above."
...
printf '\nflatpak: built from the manifest, installed, and probed in its own sandbox\n'
